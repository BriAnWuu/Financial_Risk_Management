...
Risk mapping approach with Delta-normal method. **Mapping** is a key step that replaces the actual complicated portfolio with a simpler portfolio of approximately the same risk. Some shortcomings of computing the risks of complicated portfolios includes:

1. The portfolio standard deviations can be difficult to compute, standard deviations change as the underlying asset value changes.

2. The portfolio includes too many positions, where computing the covariance matrix of thousands of instruments is not feasible.
...
Delta-normal approach assumes normal distributions for market factors, which means we have a poor fit to the tail (our risk might be underestimated). Another limitation is that Delta-normal method relies on linear (delta) approximation that is only reasonable for portfolios with limited options.  
We will introduce Filtered Historical Simulation ...  
